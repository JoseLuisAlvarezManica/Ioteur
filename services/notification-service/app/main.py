...
def _on_device_disconnected(channel, method, properties, body: bytes) -> None:
    try:
        future = asyncio.run_coroutine_threadsafe(
            handle_device_disconnected(body), _main_loop
        )
        future.result(timeout=30)
        channel.basic_ack(delivery_tag=method.delivery_tag)
    except Exception as exc:
        logger.error(
            "Unhandled error processing device.disconnected: %s",
            exc,
            extra={"event": "notification.device_disconnected.handler_error"},
        )
        channel.basic_nack(delivery_tag=method.delivery_tag, requeue=False)


def _on_system_error(channel, method, properties, body: bytes) -> None:
    request_id = (properties.headers or {}).get("request_id") or str(uuid.uuid4())
    try:
        future = asyncio.run_coroutine_threadsafe(
            handle_system_error(body, request_id), _main_loop
        )
        future.result(timeout=30)
        channel.basic_ack(delivery_tag=method.delivery_tag)
    except Exception as exc:
        logger.error(
            "Unhandled error processing system.error: %s",
            exc,
            extra={"event": "notification.system_error.handler_error"},
        )
        channel.basic_nack(delivery_tag=method.delivery_tag, requeue=False)
...
async def lifespan(app: FastAPI):
    ...
    logger.info("Starting up Notification Service", extra={"event": "startup"})
    await mongo_connect()
    start_subscriber(
        exchange="ioteur",
        queue="notification.device.disconnected",
        routing_key="device.disconnected",
        on_message=_on_device_disconnected,
    )
    start_subscriber(
        exchange="ioteur",
        queue="notification.system.error",
        routing_key="system.error",
        on_message=_on_system_error,
    )
    yield
    stop_subscriber()
    await mongo_disconnect()
    logger.info("Shutting down Notification Service", extra={"event": "shutdown"})
